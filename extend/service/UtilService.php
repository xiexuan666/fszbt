...
//解密时会通过这个密匙验证数据完整性
     ...
     * @return array
     */
    public static function userListTier($data, $spread_uid = 0, $field = 'spread_uid', $pk = 'uid', $html = '|-----',$level = 0,  $clear = true)
    {
        static $list = [];
        if ($clear) $list = [];
        foreach ($data as $k => $res) {
            if ($res[$field] == $spread_uid) {
                $res['html'] = str_repeat($html, $level);
                $list[] = $res;
                unset($data[$k]);
                self::userListTier($data, $res[$pk], $field, $pk, $html, $level + 1, false);
            }
        }
        return $list;
    }
