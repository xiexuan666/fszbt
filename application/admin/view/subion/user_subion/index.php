                    ...
                    <div class="layui-carousel layadmin-carousel layadmin-shortcut" lay-anim="" lay-indicator="inside" lay-arrow="none" style="background:none">
                        <div class="layui-card-body">
                            <div class="layui-row layui-col-space10 layui-form-item">
                                ...
                                <div class="layui-col-lg12">
                                    <label class="layui-form-label">订阅单号:</label>
            ...
            <div class="layui-card">
                ...
                <div class="layui-card-body">
                    ...
                    <script type="text/html" id="info">
                        <p>
                            {{#  if(d._info.image){ }}
                            <span><img style="width: 30px;height: 30px;margin:0;cursor: pointer;" src="{{d._info.image}}"></span>
                            {{# } }}
                            {{#  if(d._info.name){ }}
                            <span>{{d._info.name}}</span>
                            {{# } }}
                        </p>
                    </script>
...
<script>
    layList.tableList('List',"{:Url('order_list',['real_name'=>$real_name])}",function (){
        return [
            {field: 'order_id', title: '订阅号', sort: true,event:'order_id',width:'14%',templet:'#order_id'},
            {field: 'nickname', title: '用户信息',templet:'#userinfo',width:'10%'},
            {field: 'info', title: '订阅信息',templet:"#info"},
            {field: 'pay_price', title: '实际支付',width:'8%'},
            {field: 'paid', title: '支付状态',templet:'#paid',width:'8%'},
            {field: 'status', title: '订阅状态',templet:'#status',width:'8%'},
            {field: 'add_time', title: '订阅时间',width:'12%',sort: true},
    ...
    layList.tool(function (event,data,obj) {
        switch (event) {
            case 'marke':
                var url =layList.U({c:'subion.user_subion',a:'remark'}),
                    id=data.id,
                    make=data.remark;
                ...
                break;
            case 'danger':
                var url =layList.U({c:'subion.user_subion',a:'take_delivery',p:{id:data.id}});
    ...
    require(['vue'],function(Vue) {
        new Vue({
            ...
            data: {
                ...
                orderType: [
                    {name: '全部', value: ''},
                    {name: '招聘订阅', value: 1,count:orderCount.job},
                    {name: '简历订阅', value: 2,count:orderCount.resume},
                    {name: '招商订阅', value: 3,count:orderCount.supply},
                    {name: '资讯订阅', value: 4,count:orderCount.article},
                    {name: '干货订阅', value: 5,count:orderCount.knowledge},
                ],
                orderStatus: [
                    {name: '全部', value: ''},
                    {name: '未支付', value: 0,count:orderCount.wz},
                    {name: '交易完成', value: 4,count:orderCount.jy,class:true}
                ],
                dataList: [
                    {name: '全部', value: ''},
            ...
            methods: {
                ...
                getBadge:function() {
                    var that=this;
                    layList.basePost(layList.Url({c:'subion.user_subion',a:'getBadge'}),this.where,function (rem) {
                        that.badge=rem.data;
                    });
                },
                search:function () {
                ...
                excel:function () {
                    this.where.excel=1;
                    location.href=layList.U({c:'subion.user_subion',a:'order_list',q:this.where});
                }
            },
            mounted:function () {
                var that=this;
