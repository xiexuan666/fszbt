...
<div id="store-attr" class="mp-form" v-cloak="">
    <i-Form :label-width="100" style="width: 100%" v-show="hidden == false">
        <Form-Item>
            <Row>
                <i-Col span="5">
                    <i-Button type="dashed" long @click="hiddenBool" icon="plus-round">设置阶梯价格</i-Button>
                </i-Col>
            </Row>
        </Form-Item>
    </i-Form>
    <i-Form :label-width="100" style="width: 100%" v-show="hidden == true">
        <!--<Form-Item
                :label="'标题:'">
            <Row>
                <i-Col style="position: relative;margin-right: 6px" span="5"
                       ...
                       :key="index">
                    <i-Input type="text" v-model="item.value" placeholder="输入标题"></i-Input>
                ...
                <i-Col span="5">
                    <i-Button type="dashed" long @click="handleAdd" icon="plus-round">确认</i-Button>
                </i-Col>
            </Row>
        </Form-Item>-->
        <Form-Item v-show="item.attrHidden == true"
            ...
            <Row>
                ...
                <i-Col span="5">
                    <i-Input type="text" v-model="item.detailValue" placeholder="比如：10套以上"></i-Input>
                </i-Col>
                <i-Col span="5">
                    <i-Button type="primary" style="margin-left: 6px" @click="attrAdd(item)"> 确认添加 </i-Button>
                </i-Col>
            </Row>
        </Form-Item>
        <Form-Item v-show="hidden == true" style="width: 100%;">
            <Row style="margin: 0 88px 0 20px">
                <i-Col span="24">
                    <i-Button type="primary" long @click="addGoods(true)">点击生成价格设置表</i-Button>
                </i-Col>
            </Row>
        </Form-Item>
                ...
                <Form-Item>
                    <Row>
                        ...
                        <i-Col span="5" style="margin-right: 3px">
                            <span :class="attr.check ? 'check':''">数量:</span><i-Input placeholder="请输入数量" v-model="attr.sales" style="width: 68%" :number="true"></i-Input>
                        </i-Col>
                        <i-Col span="5" style="margin-right: 3px">
                            <span :class="attr.check ? 'check':''">金额:</span><i-Input placeholder="请输入金额" v-model="attr.price" style="width: 68%" :number="true"></i-Input>
                        </i-Col>
            ...
            <Form-Item>
                <Row>
                    ...
                    <i-Col span="2" offset="1">
                        <i-Button type="error" @click="clear">清空所有设置</i-Button>
                    </i-Col>
                </Row>
            </Form-Item>
        </template>
            ...
            methods: {
                ...
                handleRemove (index) {
                    if(this.items.length > 1)
                        ...
                        $eb.message('error','请设置至少一个标题');
                },
                attrRemove(item,k){
                    if(1==item.detail.length){
                        $eb.message('error','请设置至少一个套餐');
                        return false;
                    }
