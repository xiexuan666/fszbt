...
<!--                        <button class="layui-btn layui-btn-sm layui-btn-normal" type="button" data-type="set_info"><i class="fa fa-check-circle-o"></i>发送站内消息</button>-->
                    ...
                    <script type="text/html" id="nickname">
                        ...
                        <p style="color:#dab176">{{d.explain}}</p>
                        {{# } }}
                    </script>
                    ...
                    <script type="text/html" id="is_auth">
                        <input type='checkbox' name='is_auth' lay-skin='switch' value="{{d.uid}}" lay-filter='is_auth' lay-text='通过|未过'  {{ d.is_auth == 1 ? 'checked' : '' }}>
                    </script>
                    ...
                    <script type="text/html" id="license">
                        {{# if(d.license){ }}
                        <img style="cursor: pointer;" lay-event="open_license" src="{{d.license}}">
                        {{# } }}
                    </script>

                    <script type="text/html" id="positiveid">
                        {{# if(d.positiveid){ }}
                        <img style="cursor: pointer;" lay-event="open_positiveid" src="{{d.positiveid}}"><br>
                        {{# } }}
                        {{# if(d.backid){ }}
                        <img style="cursor: pointer;" lay-event="open_backid" src="{{d.backid}}"><br>
                        {{# } }}
                        {{# if(d.hold){ }}
                        <img style="cursor: pointer;" lay-event="open_hold" src="{{d.hold}}">
                        {{# } }}
                    </script>
                </div>
            </div>
...
<script>
    ...
    layList.tableList('userList',"{:Url('get_user_list')}",function () {
        return [
                {type:'checkbox'},
                {field: 'uid', title: '编号', width:'5%',event:'uid',align:'center'},
                {field: 'avatar', title: '头像', event:'open_image', width: '6%',align:'center', templet: '<p><img class="avatar" style="cursor: pointer" class="open_image" data-image="{{d.avatar}}" src="{{d.avatar}}" alt="{{d.nickname}}"></p>'},
                {field: 'nickname', title: '姓名',templet:'#nickname'},


                {field: 'image', title: '企业认证',templet:'#license',align:'center'},
                {field: 'image', title: '个人认证',templet:'#positiveid',align:'center'},
                ...
                {field: 'is_auth', title: '认证',templet:"#is_auth",align:'center',width:'7%'},
                {field: 'status', title: '状态',templet:"#checkboxstatus",align:'center',width:'7%'},
    ...
    layList.tool(function (event,data) {
        ...
        switch (layEvent){
            ...
            case 'see':
                ...
                break;
            case 'open_license':
                $eb.openImage(data.license);
                break;
            case 'open_positiveid':
                $eb.openImage(data.positiveid);
                break;
            case 'open_backid':
                $eb.openImage(data.backid);
                break;
            case 'open_hold':
                $eb.openImage(data.hold);
                break;
        }
    });
//    layList.sort('uid');
    ...
    layList.switch('is_auth',function (odj,value,name) {
        if(odj.elem.checked==true){
            layList.baseGet(layList.Url({a:'set_auth',p:{is_auth:1,uid:value}}),function (res) {
                layList.msg(res.msg);
            });
        }else{
            layList.baseGet(layList.Url({a:'set_auth',p:{is_auth:0,uid:value}}),function (res) {
                layList.msg(res.msg);
            });
        }
    });
