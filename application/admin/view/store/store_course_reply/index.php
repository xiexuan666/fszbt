                        ...
                        <div class="grid-demo">
                            <div class="layui-card homepage-bottom">
                                <div class="layui-card-body clearfix">
                                    ...
                                    <div class="layadmin-privateletterlist-item layui-col-md6 layui-col-xs6 layui-col-lg6" :class="where.producr_id==item.id ? 'on':'' " v-for="item in courseImaesList" @click="where.producr_id=item.id">
                                ...
                                <div class="layui-row producr-load clearfix">
                                    <a href="javascript:;" class="layui-btn" v-text="course.loadTitle" @click="loadList(0)">更多</a>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        new Vue({
            ...
            methods:{
                ...
                getProductImaesList:function () {
                    var that=this;
                    if(that.course.loading) return;
                    if(that.course.loadend) return;
                    that.course.loadTitle='加载中';
                    layList.baseGet(layList.U({a:'get_course_imaes_list',q:that.where}),function (res) {
                        var list=res.data;
                        var loadend=list.length < that.where.limit;
                        that.where.page=that.where.page+1;
                        that.course.loading=false;
                        that.course.loadend=loadend;
                        that.course.loadTitle=loadend ? '已全部加载' : '加载更多';
                        that.courseImaesList.push.apply(that.courseImaesList,list);
                        that.$set(that,'courseImaesList',that.courseImaesList);
                        that.slitherMonitor();
                    },function (res) {
                        that.course.loading=false;
                        that.course.loadTitle='加载更多';
                    });
                },
                getMessageList:function (isFa) {
                    ...
                    layList.baseGet(layList.U({a:'get_course_reply_list',q:that.where}),function (res) {
