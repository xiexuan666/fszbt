...
use app\admin\model\dealers\DealersCategory as CategoryModel;
...
class Dealers extends ModelBasic
{
    ...
    public static function getAll($where = array()){
        ...
        return self::page($model,function($item){
            ...
            $item['catename'] = Db::name('DealersCategory')->where('id',$item['cid'])->value('title');
        },$where);
    }
    ...
    public static function setContent($id,$content){
        $count = Db::name('DealersContent')->where('nid',$id)->count();
        $data['nid'] = $id;
        $data['content'] = $content;
        if($count){
            $res = Db::name('DealersContent')->where('nid',$id)->setField('content',$content);
            if($res !== false) $res = true;
        }
        else
            $res = Db::name('DealersContent')->insert($data);
        return $res;
    }
    ...
    public static function merchantPage($where = array()){
        ...
        return self::page($model,function($item){
            $item['content'] = Db::name('DealersContent')->where('nid',$item['id'])->value('content');
        },$where);
    }
    ...
    public static function getArticleList($id = '',$field = 'title,author,image_input,synopsis,id'){
        ...
        foreach ($list as $k=>$v){
            $list[$k]['content'] = Db::name('DealersContent')->where('nid',$v['id'])->value('content');
        }
        return $list;
    }
}