 ...
 * 人才分类 model
 * Class Position
 * @package app\admin\model\position
 */
class Position extends ModelBasic
{
    ...
    public static function getList($where){
        ...
        foreach ($data as &$item){
            if($item['pid']){
                $item['pid_name'] = self::where('id',$item['pid'])->value('name');
            }else{
                $item['pid_name'] = '顶级';
            }
        }
    ...
    public static function systemPage($where,$isAjax=false){
        ...
        else if($where['pid']=='' && $where['name']=='') $model = $model->where('pid',0);
     ...
     * @return array
     */
    public static function getPosition($id,$field){
        $res = PositionModel::where('status',1)->where('hide',0)->column($field,'id');
        $new_res = array();
        foreach ($res as $k=>$v){
