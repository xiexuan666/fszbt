 ...
 * @package app\admin\model\supply
 */
class SupplyCategory extends ModelBasic
{
    use ModelTrait;
     ...
     * @return bool
     */
    public static function delSupplyCategory($id)
    {
        if(count(self::getSupply($id,'*'))>0)
     ...
     * @return array
     */
    public static function getSupply($id,$field){
        $res = SupplyModel::where('status',1)->where('hide',0)->column($field,'id');
        $new_res = array();
        foreach ($res as $k=>$v){
