    ...
    public static function getUserList($where){
        ...
        $list = $model->alias('u')
            ...
            ->each(function ($item){
                ...
                if($item['last_time']) $item['last_time'] = date('Y-m-d H:i',$item['last_time']);//最近一次访问日期
                ...
                $item['vip_name']=false;
                $levelinfo=UserLevel::where(['uid'=>$item['uid'],'is_del'=>0])->order('id desc')->field(['level_id','is_forever','valid_time'])->find();
                if($levelinfo){
                    ...
                    $item['explain'] = SystemUserLevel::where('id',$levelinfo['level_id'])->value('explain');
                }
            });//->toArray();
...
//        echo self::getLastSql();die;
    ...
    public static function getTierList($model = null)
    {
        ...
        return Util::userListTier($model->select()->toArray());
    }
}