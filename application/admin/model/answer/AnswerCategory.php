 ...
 * @package app\admin\model\answer
 */
class AnswerCategory extends ModelBasic
{
    use ModelTrait;
     ...
     * @return bool
     */
    public static function delAnswerCategory($id)
    {
        if(count(self::getAnswer($id,'*'))>0)
     ...
     * @return array
     */
    public static function getAnswer($id,$field){
        $res = AnswerModel::where('status',1)->where('hide',0)->column($field,'id');
        $new_res = array();
        foreach ($res as $k=>$v){
