 ...
 * @package app\admin\model\knowledge
 */
class KnowledgeCategory extends ModelBasic
{
    use ModelTrait;
     ...
     * @return bool
     */
    public static function delKnowledgeCategory($id)
    {
        if(count(self::getKnowledge($id,'*'))>0)
     ...
     * @return array
     */
    public static function getKnowledge($id,$field){
        $res = KnowledgeModel::where('status',1)->where('hide',0)->column($field,'id');
        $new_res = array();
        foreach ($res as $k=>$v){
