...
class StoreCourseReply extends ModelBasic
{
    ...
    public static function valiWhere($where,$alias='',$model=null)
    {
        ...
        if(isset($where['producr_id']) && $where['producr_id']!=0) $model=$model->where('course_id',$where['producr_id']);
        return $model->where("{$alias}is_del",0);
    }

    public static function getCourseImaesList($where)
    {
        $list=self::valiWhere($where,'a')->group('p.id')->join('__WECHAT_USER__ u','u.uid=a.uid')->join("__STORE_PRODUCT__ p",'a.course_id=p.id')->field(['p.id','p.image','p.store_name','p.price'])->page($where['page'],$where['limit'])->select();
    ...
    public static function getCourseReplyList($where)
    {
        $data=self::valiWhere($where,'a')->join("__STORE_PRODUCT__ p",'a.course_id=p.id')
        ...
        $count=self::valiWhere($where,'a')->join('__WECHAT_USER__ u','u.uid=a.uid')->join("__STORE_PRODUCT__ p",'a.course_id=p.id')->count();
    ...
    public static function systemPage($where){
        ...
        if($where['course_id'])  $model = $model->where('r.course_id',$where['course_id']);
        $model = $model->alias('r')->join('__WECHAT_USER__ u','u.uid=r.uid');
        $model = $model->join('__STORE_PRODUCT__ p','p.id=r.course_id');
