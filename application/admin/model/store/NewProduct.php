...
class NewProduct extends ModelBasic
{
    ...
    public static function getModelObject($where=[]){
        $model=new self();
        $model=$model->alias('p')->join('NewProductAttrValue pav','p.id=pav.product_id','LEFT');
        if(!empty($where)){
            ...
            if(isset($where['name']) && $where['name']!=''){
                $model = $model->where('p.name|p.id','LIKE',"%$where[name]%");
            }
    ...
    public static function ProductList($where){
        ...
        $model=$model->order('is_pay desc,is_hot desc,sort desc');
        ...
        foreach ($data as &$item){
            ...
            $item['collect'] = NewProductRelation::where('product_id',$item['id'])->where('type','collect')->count();//收藏
            $item['like'] = NewProductRelation::where('product_id',$item['id'])->where('type','like')->count();//点赞
            $item['browse'] = self::getStock($item['id'])>0?self::getStock($item['id']):$item['browse'];//库存
            $item['stock_attr'] = self::getStock($item['id'])>0 ? true : false;//库存
            $item['sales_attr'] = self::getSales($item['id']);//属性销量
            $item['visitor'] = Db::name('new_visit')->where('product_id',$item['id'])->where('product_type','product')->count();
    ...
    public static function getChatrdata($type,$data){
        ...
        $list=self::getModelTime(compact('data'),$model)
            ...
            ->each(function($item) use($data){
                $item['collect']=self::getModelTime(compact('data'),new NewProductRelation)->where(['type'=>'collect'])->count();
                $item['like']=self::getModelTime(compact('data'),new NewProductRelation)->where(['type'=>'like'])->count();
            })->toArray();
        $chatrList=[];
        $datetime=[];
        $data_item=[];
    ...
    public static function getnegativelist($where){
        $list=self::alias('s')->join('NewProductReply r','s.id=r.product_id')
            ...
            ->select();
        if(count($list)) $list=$list->toArray();
        $count=self::alias('s')->join('NewProductReply r','s.id=r.product_id')->group('r.product_id')->where('r.product_score',1)->count();
        return ['count'=>$count,'data'=>$list];
    }
    ...
    {
        return NewProductAttrValue::where(['product_id'=>$productId])->sum('stock');
    }
    //获取总销量
    public static function getSales($productId)
    {
        return NewProductAttrValue::where(['product_id'=>$productId])->sum('sales');
    }
