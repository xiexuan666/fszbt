...
class StoreCourse extends ModelBasic
{
    ...
    public static function getModelObject($where=[]){
        $model=new self();
        $model=$model->alias('p')->join('StoreCourseAttrValue pav','p.id=pav.course_id','LEFT');
        if(!empty($where)){
...
//                $model = $model->whereOr('p.cate_id','LIKE',["%$catid%",$catidab]);
    ...
    public static function CourseList($where){
        ...
        foreach ($data as &$item){
            ...
            $item['collect'] = StoreCourseRelation::where('course_id',$item['id'])->where('type','collect')->count();//收藏
            $item['like'] = StoreCourseRelation::where('course_id',$item['id'])->where('type','like')->count();//点赞
    ...
    public static function getChatrdata($type,$data){
        ...
        $list=self::getModelTime(compact('data'),$model)
            ...
            ->each(function($item) use($data){
                $item['collect']=self::getModelTime(compact('data'),new StoreCourseRelation)->where(['type'=>'collect'])->count();
                $item['like']=self::getModelTime(compact('data'),new StoreCourseRelation)->where(['type'=>'like'])->count();
            })->toArray();
        $chatrList=[];
        $datetime=[];
        $data_item=[];
    ...
    public static function getMaxList($where){
        ...
        $model=StoreOrder::alias('a')->join('StoreOrderCartInfo c','a.id=c.oid')->join('__STORE_PRODUCT__ b','b.id=c.course_id');
        $list=self::getModelTime($where,$model,'a.add_time')->group('c.course_id')->order('p_count desc')->limit(10)
            ->field(['count(c.course_id) as p_count','b.store_name','sum(b.price) as sum_price'])->select();
    ...
    public static function ProfityTop10($where){
        $classs=['layui-bg-red','layui-bg-orange','layui-bg-green','layui-bg-blue','layui-bg-cyan'];
        $model=StoreOrder::alias('a')->join('StoreOrderCartInfo c','a.id=c.oid')->join('__STORE_PRODUCT__ b','b.id=c.course_id');
        $list=self::getModelTime($where,$model,'a.add_time')->group('c.course_id')->order('profity desc')->limit(10)
            ->field(['count(c.course_id) as p_count','b.store_name','sum(b.price) as sum_price','(b.price-b.cost) as profity'])
            ->select();
        if(count($list)) $list=$list->toArray();
        $maxList=[];
        $sum_count=0;
    ...
    public static function getnegativelist($where){
        $list=self::alias('s')->join('StoreCourseReply r','s.id=r.course_id')
            ->field('s.id,s.store_name,s.price,count(r.course_id) as count')
            ->page((int)$where['page'],(int)$where['limit'])
            ->where('r.course_score',1)
            ->order('count desc')
            ->group('r.course_id')
            ->select();
        if(count($list)) $list=$list->toArray();
        $count=self::alias('s')->join('StoreCourseReply r','s.id=r.course_id')->group('r.course_id')->where('r.course_score',1)->count();
        return ['count'=>$count,'data'=>$list];
    }
    public static function TuiCourseList(){
        $perd=StoreOrder::alias('s')->join('StoreOrderCartInfo c','s.id=c.oid')
            ->field('count(c.course_id) as count,c.course_id as id')
            ->group('c.course_id')
            ->where('s.status',-1)
            ->order('count desc')
            ->limit(10)
    ...
    //编辑库存
    public static function changeStock($stock,$courseId)
    {
        return self::edit(compact('stock'),$courseId);
    }
    //获取库存数量
    public static function getStock($courseId)
    {
        return StoreCourseAttrValue::where(['course_id'=>$courseId])->sum('stock');
    }
    //获取总销量
    public static function getSales($courseId)
    {
        return StoreCourseAttrValue::where(['course_id'=>$courseId])->sum('sales');
    }
    ...
    public static function setWhere($where){
        ...
        $model=self::getModelTime($time, Db::name('store_cart')->alias('a')->join('__STORE_PRODUCT__ b','a.course_id=b.id'),'a.add_time');
    ...
    public static function getSaleslists($where){
        $data=self::setWhere($where)->where('a.is_pay',1)
            ->group('a.course_id')
            ->field(['sum(a.cart_num) as num_course','b.store_name','b.image','b.price','b.id'])
            ->order('num_course desc')
            ->page((int)$where['page'],(int)$where['limit'])
            ->select();
        $count=self::setWhere($where)->where('a.is_pay',1)->group('a.course_id')->count();
        foreach ($data as &$item){
            $item['sum_price']=bcmul($item['num_course'],$item['price'],2);
        }
        return compact('data','count');
    }
    public static function SaveCourseExport($where){
        ...
        foreach ($list as $item){
            $export[]=[
                ...
                $item['store_name'],
                $item['price'],
                bcmul($item['num_course'],$item['price'],2),
                $item['num_course'],
            ];
        }
     ...
     *  $where 条件
     */
    public static function getCourseBadgeList($id,$where){
        $data['data']=$where;
        $list=self::setWhere($data)
            ->field(['sum(a.cart_num) as num_course','b.id','b.price'])
            ->where('a.is_pay',1)
            ->group('a.course_id')
            ->order('num_course desc')
            ->select();
        ...
        foreach ($list as $key=>$item){
            if($item['id']==$id){
                $ranking=$key+1;
                $xiaoliang=$item['num_course'];
            }
            $value['sum_price']=$item['price']*$item['num_course'];
            $value['id']=$item['id'];
            $list_price[]=$value;
        }
        //排序
        ...
        return [
            ...
            [
                'name'=>'点赞次数',
                'field'=>'个',
                'count'=>Db::name('store_course_relation')->where('course_id',$id)->where('type','like')->count(),
                'background_color'=>'layui-bg-blue',
            ],
            [
                'name'=>'销售总额',
     ...
     *
     */
    public static function getCourseCurve($where){
        $list=self::setWhere($where)
            ->where('a.course_id',$where['id'])
            ->where('a.is_pay',1)
    ...
    public static function getSalelList($where){
        return self::setWhere($where)
            ->where(['a.course_id'=>$where['id'],'a.is_pay'=>1])
