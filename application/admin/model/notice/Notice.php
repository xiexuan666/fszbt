 ...
 * Class Notice
 */
class Notice extends ModelBasic
{
    use ModelTrait;
    ...
    public static function getAll($where = array()){
        ...
        return self::page($model,function($item){
            ...
            $item['catename'] = Db::name('NoticeCategory')->where('id',$item['cid'])->value('title');
        },$where);
    }
    ...
    public static function setContent($id,$content){
        $count = Db::name('NoticeContent')->where('nid',$id)->count();
        $data['nid'] = $id;
        $data['content'] = $content;
        if($count){
            $res = Db::name('NoticeContent')->where('nid',$id)->setField('content',$content);
            if($res !== false) $res = true;
        }
        else
            $res = Db::name('NoticeContent')->insert($data);
        return $res;
    }
    ...
    public static function merchantPage($where = array()){
        ...
        return self::page($model,function($item){
            $item['content'] = Db::name('NoticeContent')->where('nid',$item['id'])->value('content');
        },$where);
    }
     ...
     * @throws \think\exception\DbException
     */
    public static function getNoticeList($id = '',$field = 'title,author,image_input,synopsis,id'){
        $list = self::where('id','IN',$id)->field($field)->select();
        foreach ($list as $k=>$v){
            $list[$k]['content'] = Db::name('NoticeContent')->where('nid',$v['id'])->value('content');
        }
        return $list;
    }
}