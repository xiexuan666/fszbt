...
class Article extends ModelBasic
{
    ...
    public static function getList($where){
        ...
        foreach ($data as &$item){
            ...
            $item['count'] = UserSubion::where('subion_id',$item['id'])->where('type','article')->where('paid',1)->count();
            $item['sum'] = UserSubion::where('subion_id',$item['id'])->where('type','article')->where('paid',1)->sum('pay_price');
        }
