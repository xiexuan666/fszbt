...
namespace app\admin\model\subion;
...
class UserSubion extends ModelBasic
{
    ...
    public static function orderCount(){
        ...
        $data['job']=self::where('type','job')->count();
        $data['resume']=self::where('type','resume')->count();
        $data['supply']=self::where('type','supply')->count();
        $data['article']=self::where('type','article')->count();
        $data['knowledge']=self::where('type','knowledge')->count();
        return $data;
    }
            ...
            if($item['type']){
                switch ($item['type']){
                    case 'job':
                        ...
                        //检索招聘
                        $_info = Db::name('job_position')->where('id',$item['subion_id'])->field('uid,salary,address,position')->find();
                        $_position = Db::name('position')->where('id',$_info['position'])->field('name')->find();
                        $avatar = User::where('uid',$_info['uid'])->value('avatar');
                        if(!$avatar) $avatar = $logoUrl;

                        $item['_info'] = array('image'=>$avatar,'name'=>$_position['name'].' · '.$_info['address'].' · '.$_info['salary']);
                        break;
                    case 'resume':
                        $item['pink_name'] = '[简历订阅]';
                        $item['color'] = '#00f';
                        ...
                        //检索简历
                        $_info = Db::name('resume')->where('id',$item['subion_id'])->field('uid,name,status,phone')->find();
                        $_expect = Db::name('resume_expect')->where('id',$_info['uid'])->field('position,salary')->find();
                        $_position = Db::name('position')->where('id',$_expect['position'])->field('name')->find();
                        $avatar = User::where('uid',$_info['uid'])->value('avatar');
                        if(!$avatar) $avatar = $logoUrl;

                        $item['_info'] = array('image'=>$avatar,'name'=>$_info['name'].' · '.$_position['name'].' · '.$_expect['salary']);
                        break;
                    case 'supply':
                        $item['pink_name'] = '[招商订阅]';
                        $item['color'] = '#f0f';
                        ...
                        //检索招商
                        $_info = Db::name('supply')->where('id',$item['subion_id'])->field('uid,title,image')->find();
                        $avatar = $_info['image'];
                        if(!$avatar) $avatar = $logoUrl;

                        $item['_info'] = array('image'=>$avatar,'name'=>$_info['title']);
                        break;
                    case 'article':
                        $item['pink_name'] = '[资讯订阅]';
                        $item['color'] = '#f0f';
                        ...
                        //检索资讯
                        $_info = Db::name('article')->where('id',$item['subion_id'])->field('uid,title,image')->find();
                        $avatar = $_info['image'];
                        if(!$avatar) $avatar = $logoUrl;
                    ...
                    case 'knowledge':
                        ...
                        //检索干货
                        $_info = Db::name('knowledge')->where('id',$item['subion_id'])->field('uid,title,image')->find();
                        $avatar = $_info['image'];
                        if(!$avatar) $avatar = $logoUrl;
...
HTML;
    ...
    public static function SaveExcel($list){
        ...
        foreach ($list as $index=>$item){
            ...
            switch ($item['type']){
                case 'job':
                    ...
                    $item['_info'] = array('image'=>$avatar,'name'=>$_position['name'].' · '.$_info['address'].' · '.$_info['salary'],'pink_name'=>'[招聘订阅]');
                    break;
                case 'resume':
                    //检索简历
                    $_info = Db::name('resume')->where('id',$item['subion_id'])->field('uid,name,status,phone')->find();
                    ...
                    $item['_info'] = array('image'=>$avatar,'name'=>$_info['name'].' · '.$_position['name'].' · '.$_expect['salary'],'pink_name'=>'[简历订阅]');
                    break;
                case 'supply':
                    //检索招商
                    $_info = Db::name('supply')->where('id',$item['subion_id'])->field('uid,title,image')->find();
                    ...
                    $item['_info'] = array('image'=>$avatar,'name'=>$_info['title'],'pink_name'=>'[招商订阅]');
                    break;
                case 'article':
                    //检索资讯
                    $_info = Db::name('article')->where('id',$item['subion_id'])->field('uid,title,image')->find();
                    ...
                    $item['_info'] = array('image'=>$avatar,'name'=>$_info['title'],'pink_name'=>'[资讯订阅]');
                    break;
                case 'knowledge':
                    //检索干货
                    $_info = Db::name('knowledge')->where('id',$item['subion_id'])->field('uid,title,image')->find();
                    ...
                    $item['_info'] = array('image'=>$avatar,'name'=>$_info['title'],'pink_name'=>'[干货订阅]');
                    break;
                default:
                    $item['_info'] = array('pink_name'=>'[其他订阅]');
                    break;
            }
        ...
        PHPExcelService::setExcelHeader(['订阅单号','支付方式','订阅总数','订阅总价','支付金额','订阅人','订阅信息','支付状态'])
            ->setExcelTile('订阅导出','订阅信息'.time(),' 生成时间：'.date('Y-m-d H:i:s',time()))
    ...
    public static function getOrderWhere($where,$model,$aler='',$join=''){
        ...
        if(isset($where['type'])){
            switch ($where['type']){
                case 1:
                    $model = $model->where($aler.'type',"job");
                    break;
                case 2:
                    $model = $model->where($aler.'type',"resume");
                    break;
                case 3:
                    $model = $model->where($aler.'type',"supply");
                    break;
                case 4:
                    $model = $model->where($aler.'type',"article");
                    break;
                case 5:
                    $model = $model->where($aler.'type',"knowledge");
                    break;
                default:
                    $model = $model->where($aler.'type',"IN",['job','resume','supply','article','knowledge']);
                    break;
            }
        }
