...
namespace app\admin\model\consultant;
 ...
 * Class Consultant
 */
class Consultant extends ModelBasic
{
    use ModelTrait;
    ...
    public static function getAll($where = array()){
        ...
        return self::page($model,function($item){
            ...
            $item['content'] = Db::name('ConsultantContent')->where('nid',$item['id'])->value('content');
            $item['catename'] = Db::name('ConsultantCategory')->where('id',$item['cid'])->value('title');
        },$where);
    }
    ...
    public static function setContent($id,$content){
        $count = Db::name('ConsultantContent')->where('nid',$id)->count();
        $data['nid'] = $id;
        $data['content'] = $content;
        if($count){
            $res = Db::name('ConsultantContent')->where('nid',$id)->setField('content',$content);
            if($res !== false) $res = true;
        }
        else
            $res = Db::name('ConsultantContent')->insert($data);
        return $res;
    }
    ...
    public static function getArticleList($id = '',$field = 'title,author,image_input,synopsis,id'){
        ...
        foreach ($list as $k=>$v){
            $list[$k]['content'] = Db::name('ConsultantContent')->where('nid',$v['id'])->value('content');
        }
        return $list;
    }
}