 ...
 * @package app\admin\model\consultant
 */
class ConsultantCategory extends ModelBasic
{
    use ModelTrait;
     ...
     * @return bool
     */
    public static function delConsultantCategory($id)
    {
        if(count(self::getConsultant($id,'*'))>0)
     ...
     * @return array
     */
    public static function getConsultant($id,$field){
        $res = ConsultantModel::where('status',1)->where('hide',0)->column($field,'id');
        $new_res = array();
        foreach ($res as $k=>$v){
