...
class StoreRaisedata extends ModelBasic{
    ...
    public static function systemPage($where){
        ...
        $model = new self;
        $model = $model->alias('p');
        $model = $model->field('p.*,c.title,c.sales,c.unit_name,c.stop_time,u.nickname,u.avatar');
        if($where['data'] !== ''){
        ...
        $model = $model->join('__STORE_RAISE__ c','c.id=p.cid');
     ...
     * @return int|string
     */
    public static function getCountPeopleRaisedata($combinationId = 0){
