 ...
 */
class StoreRaise extends ModelBasic
{
    use ModelTrait;
    ...
    public static function setWhere($where,$model=null){
        ...
        return $model->order('c.sort desc')->where('c.is_del',0);
    }
    /**
     * @param $where
     * @return array
     ...
     * @return mixed
     */
    public static function getVisitPeople($raiseId = 0,$productType = 'raise'){
        $model = Db::name('store_visit');
        $model = $model->where('product_id',$raiseId);
    ...
    public static function getStatistics(){
        ...
        $statistics['visitCount'] = Db::name('store_visit')->where('product_type','raise')->count();//访客人数
     ...
     * @return int|string
     */
    public static function getRaiseCount(){
     ...
     * @return array
     */
    public static function getRaiseIdAll(){
     ...
     * @return mixed
     */
    public static function getRaiseOne($id){
        $model = new self();
        $model = $model->alias('c');
        $model = $model->join('StoreProduct s','s.id=c.product_id');
        $model = $model->field('c.*,s.price as product_price');
     ...
     * @return mixed
     */
    public static function getRaiseHost($limit = 0){
        $model = new self();
        $model = $model->alias('c');
        $model = $model->join('StoreProduct s','s.id=c.product_id');
     ...
     * @return int|mixed
     */
    public static function getRaiseStock($id,$cart_num){
     ...
     * @return mixed
     */
    public static function isValidRaise($id){
        $model = new self();
     ...
     * @return bool
     */
    public static function decRaiseStock($num,$RaiseId)
    {
        $res = false !== self::where('id',$RaiseId)->dec('stock',$num)->inc('sales',$num)->update();
        return $res;
    }
    /**
     * 拼团产品过滤条件
    ...
    public static function getbadge($where,$type){
        ...
        return [
            ...
            [
                'name'=>'拼团成功订单',
                'field'=>'件',
                'count'=>self::getModelTime($where,$StoreOrderModel)->where('raise_id','NEQ',0)->sum('total_num'),
                'content'=>'活动商品总数',
                'background_color'=>'layui-bg-green',
                'sum'=>$StoreOrderModel->where('raise_id','neq',0)->sum('total_num'),
    ...
    public static function ProfityTop10($where){
        ...
        $model = StoreOrder::alias('a')->join('__STORE_COMBINATION__ b','b.id = a.raise_id')->where('a.paid',1);
        $list=self::getModelTime($where,$model,'a.add_time')->group('a.seckill_id')->order('profity desc')->limit(10)
            ->field(['count(a.raise_id) as p_count','b.title as store_name','sum(b.price) as sum_price','(b.price-b.cost) as profity'])
            ->select();
    ...
    public static function getMaxList($where){
        $classs=['layui-bg-red','layui-bg-orange','layui-bg-green','layui-bg-blue','layui-bg-cyan'];
        $model=StoreOrder::alias('a')->join('__STORE_COMBINATION__ b','b.id=a.raise_id')->where('a.paid',1);
        $list=self::getModelTime($where,$model,'a.add_time')->group('a.raise_id')->order('p_count desc')->limit(10)
            ->field(['count(a.raise_id) as p_count','b.title as store_name','sum(b.price) as sum_price'])->select();
        if(count($list)) $list=$list->toArray();
        $maxList=[];
        $sum_count=0;
        $sum_price=0;
    ...
    public static function getBargainRefundList($where = array()){
        $model = StoreOrder::alias('a')->join('__STORE_COMBINATION__ b','b.id=a.raise_id');
        $list = self::getModelTime($where,$model,'a.add_time')->where('a.refund_status','NEQ',0)->group('a.raise_id')
            ->order('count desc')->page((int)$where['page'],(int)$where['limit'])
            ->field(['count(a.raise_id) as count','b.title as store_name','sum(b.price) as sum_price'])
            ->select();
        if(count($list)) $list=$list->toArray();
        return $list;
    }
