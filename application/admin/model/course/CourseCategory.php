...
class CourseCategory extends ModelBasic
{
     ...
     * @return bool
     */
    public static function delCourseCategory($id)
    {
        if(count(self::getCourse($id,'*'))>0)
     ...
     * @return array
     */
    public static function getCourse($id,$field){
        $res = CourseModel::where('status',1)->where('hide',0)->column($field,'id');
        $new_res = array();
        foreach ($res as $k=>$v){
