 ...
 * @package app\admin\model\company
 */
class CompanyCategory extends ModelBasic
{
    use ModelTrait;
     ...
     * @return bool
     */
    public static function delCompanyCategory($id)
    {
        if(count(self::getCompany($id,'*'))>0)
     ...
     * @return array
     */
    public static function getCompany($id,$field){
        $res = CompanyModel::where('status',1)->where('hide',0)->column($field,'id');
        $new_res = array();
        foreach ($res as $k=>$v){
