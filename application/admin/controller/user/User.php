...
class User extends AuthController
{
    ...
    /**
     * 修改user表状态
     * @param string $is_auth
     * @param int $uid
     * @param int $is_echo
     */
    public function set_auth($is_auth='',$uid=0,$is_echo=0){
        if($is_echo==0) {
            if ($is_auth == '' || $uid == 0) return Json::fail('参数错误');
            UserModel::where(['uid' => $uid])->update(['is_auth' => $is_auth,'is_examine'=>$is_auth?0:1]);
        }else{
            $uids=Util::postMore([
                ['uids',[]]
            ]);
            UserModel::destrSyatus($uids['uids'],$is_auth);
        }
        return Json::successful($is_auth==1 ? '认证成功':'取消认证成功');
    }
    /**
     * 获取user表
     *
    ...
    public function edit($uid)
    {
        ...
        $f[] = Form::select('level','会员等级',explode(',',$user->getData('level')))->setOptions(function(){
            $list = SystemUserLevel::select();
            $menus=[];
        ...
        $f[] = Form::number('spread_uid','推荐人UID',$user->getData('spread_uid'))->min(0);
        ...
        $f[] = Form::frameImageOne('license','营业执照副本',Url::build('admin/widget.images/index',array('fodder'=>'license')),$user->getData('license'))->icon('image')->width('100%')->height('500px');
        $f[] = Form::frameImageOne('positiveid','身份证正面照',Url::build('admin/widget.images/index',array('fodder'=>'positiveid')),$user->getData('positiveid'))->icon('image')->width('100%')->height('500px');
        $f[] = Form::frameImageOne('backid','身份证背面照',Url::build('admin/widget.images/index',array('fodder'=>'backid')),$user->getData('backid'))->icon('image')->width('100%')->height('500px');
        $f[] = Form::frameImageOne('hold','身份证手持照',Url::build('admin/widget.images/index',array('fodder'=>'hold')),$user->getData('hold'))->icon('image')->width('100%')->height('500px');

        $f[] = Form::radio('is_auth','通过认证',$user->getData('is_auth'))->options([['value'=>1,'label'=>'通过'],['value'=>0,'label'=>'不通过']]);
    ...
    public function update(Request $request, $uid)
    {
        ...
        if($data['level']){
            UserLevel::where(['uid'=>$uid])->delete();
            $vipinfo = SystemUserLevel::get($data['level']);
            $add_valid_time = (int)$vipinfo['valid_date']*86400;
            $uservipinfo = UserLevel::where(['uid'=>$uid,'level_id'=>$data['level']])->find();
            //检查是否购买过
            if($uservipinfo){
                $stay=0;
                //剩余时间
                ...
                UserLevel::where(['uid'=>$uid,'level_id'=>$data['level']])->update($datas);
            } else {
                $datas=[
        ...
        $edit['level'] = $vipinfo['grade'];
        $edit['is_auth'] = $data['is_auth'];
        $edit['status'] = $data['status'];
        $edit['is_promoter'] = $data['is_promoter'];
        $edit['is_examine'] = $data['is_auth'] ? 0:1;
