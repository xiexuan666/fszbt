...
class Course extends AuthController
{
    ...
    public function index($pid = 0)
    {
        ...
        $catlist = CourseCategoryModel::where('is_del',0)->select()->toArray();
        //获取分类列表
        if($catlist){
        ...
        $this->assign('cate',CourseCategoryModel::getTierList());
        $this->assign(CourseModel::getAll($where));
        return $this->fetch();
    }
    ...
    public function save(Request $request)
    {
        ...
        CourseModel::beginTrans();
        $res1 = CourseModel::set($data);
        if($res1)
            $res = true;
        else
            $res =false;
        CourseModel::checkTrans($res);
        if($res)
