 ...
 * @author: 招宝通
 */
namespace app\admin\controller\subion;
...
use think\Request;
use think\Url;
use app\admin\model\subion\UserSubion as UserSubionModel;
/**
 * 订单管理控制器 同一个订单表放在一个控制器
 * Class StoreOrder
 * @package app\admin\controller\store
 */
class UserSubion extends AuthController
{
    /**
     * @return mixed
     */
    public function index()
    {
        $this->assign([
            ...
            'orderCount'=>UserSubionModel::orderCount(),
        ]);
        return $this->fetch();
    }
    /**
    ...
    public function getBadge(){
        ...
        return JsonService::successful(UserSubionModel::getBadge($where));
    }
    /**
     * 获取订单列表
     * return json
    ...
    public function order_list(){
        ...
        return JsonService::successlayui(UserSubionModel::OrderList($where));
    }
    public function orderchart(){
        ...
        $orderCount = [
            urlencode('未支付')=>UserSubionModel::getOrderWhere($where,UserSubionModel::statusByWhere(0))->count(),
            urlencode('未发货')=>UserSubionModel::getOrderWhere($where,UserSubionModel::statusByWhere(1))->count(),
            urlencode('待收货')=>UserSubionModel::getOrderWhere($where,UserSubionModel::statusByWhere(2))->count(),
            urlencode('待评价')=>UserSubionModel::getOrderWhere($where,UserSubionModel::statusByWhere(3))->count(),
            urlencode('交易完成')=>UserSubionModel::getOrderWhere($where,UserSubionModel::statusByWhere(4))->count(),
            urlencode('退款中')=>UserSubionModel::getOrderWhere($where,UserSubionModel::statusByWhere(-1))->count(),
            urlencode('已退款')=>UserSubionModel::getOrderWhere($where,UserSubionModel::statusByWhere(-2))->count()
        ];
        $model = UserSubionModel::getOrderWhere($where,new UserSubionModel())->field('sum(total_num) total_num,count(*) count,sum(total_price) total_price,sum(refund_price) refund_price,from_unixtime(add_time,\'%Y-%m-%d\') add_time')
        ...
        $this->assign(UserSubionModel::systemPage($where,$this->adminId));
        $this->assign('price',UserSubionModel::getOrderPrice($where));
    ...
    public function edit($id)
    {
        ...
        $product = UserSubionModel::get($id);
...
//        $f[] = Form::radio('status','状态',$product->getData('status'))->options([['label'=>'开启','value'=>1],['label'=>'关闭','value'=>0]]);
    ...
    public function update(Request $request, $id)
    {
        ...
        $data['order_id'] = UserSubionModel::changeOrderId($data['order_id']);
        UserSubionModel::edit($data,$id);
    ...
    public function take_delivery($id){
        ...
        if(!UserSubionModel::edit($data,$id))
            return Json::fail(UserSubionModel::getErrorInfo('收货失败,请稍候再试!'));
        else{
            try{
    ...
    public function updateRefundY(Request $request, $id){
        ...
        $resEdit = UserSubionModel::edit($data,$id);
        if($resEdit){
            $data['type'] = $type;
    ...
    {
        if(!$oid || !($orderInfo = UserSubionModel::get($oid)))
    ...
    {
        if(!$oid || !($order = UserSubionModel::get($oid)))
            return $this->failed('订单不存在!');
    ...
    public function offline($id){
        $res = UserSubionModel::updateOffline($id);
        if($res){
            try{
