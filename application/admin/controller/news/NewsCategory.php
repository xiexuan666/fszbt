 ...
 */
class NewsCategory extends AuthController
{
    /**
     * 显示资源列表
     *
    ...
    public function getList(){
        ...
        return Json::successlayui(NewsCategoryModel::getList($where));
    }
    /**
     * 设置单个产品上架|下架
     *
    ...
    public function set_show($status='',$id=''){
        ...
        $res=NewsCategoryModel::where(['id'=>$id])->update(['status'=>(int)$status]);
        if($res){
    ...
    public function set_category($field='',$id='',$value=''){
        ...
        if(NewsCategoryModel::where(['id'=>$id])->update([$field=>$value]))
    ...
    public function save(Request $request){
        ...
        $new_id = $data['new_id'];
        unset($data['new_id']);
        $res = NewsCategoryModel::set($data);
        if(!NewsModel::saveBatchCid($res['id'],implode(',',$new_id))) return Json::fail('文章列表添加失败');
    ...
    public function edit($id){
        ...
        $article = NewsCategoryModel::get($id)->getData();
    ...
    public function update(Request $request, $id)
    {
        ...
        if($data['sort'] < 0) return Json::fail('排序不能是负数');
        $data['image'] = $data['image'][0];
        if(!NewsCategoryModel::get($id)) return Json::fail('编辑的记录不存在!');
        NewsCategoryModel::edit($data,$id);
    ...
    {
        $res = NewsCategoryModel::delNewsCategory($id);
        if(!$res)
            return Json::fail(NewsCategoryModel::getErrorInfo('删除失败,请稍候再试!'));
        else
