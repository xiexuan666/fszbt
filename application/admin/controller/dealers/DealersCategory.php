 ...
 */
class DealersCategory extends AuthController
{
    /**
     * 分类管理
     * @return mixed
    ...
    public function getList(){
        ...
        return Json::successlayui(DealersCategoryModel::getList($where));
    }
    /**
     * 显示隐藏
     * @return json
    ...
    public function set_show($status='',$id=''){
        ...
        $res = DealersCategoryModel::where(['id'=>$id])->update(['status'=>(int)$status]);
        if($res){
    ...
    public function set_category($field='',$id='',$value=''){
        ...
        if(DealersCategoryModel::where(['id'=>$id])->update([$field=>$value]))
    ...
    {
        $res = DealersCategoryModel::del($id);
        if(!$res)
            return Json::fail(DealersCategoryModel::getErrorInfo('删除失败,请稍候再试!'));
        else
    ...
    public function save(Request $request){
        ...
        $new_id = $data['new_id'];
        unset($data['new_id']);
        $res = DealersCategoryModel::set($data);
        if(!DealersModel::saveBatchCid($res['id'],implode(',',$new_id))) return Json::fail('文章列表添加失败');
    ...
    public function edit($id){
        ...
        $article = DealersCategoryModel::get($id)->getData();
    ...
    public function update(Request $request, $id)
    {
        ...
        if($data['sort'] < 0) return Json::fail('排序不能是负数');
        $data['image'] = $data['image'][0];
        if(!DealersCategoryModel::get($id)) return Json::fail('编辑的记录不存在!');
        DealersCategoryModel::edit($data,$id);
