...
use app\admin\model\store\NewProductRelation;
...
use app\admin\model\store\NewCategory as CategoryModel;
use app\admin\model\store\NewProduct as ProductModel;
use think\Url;
 ...
 */
class NewProduct extends AuthController
{
    ...
    public function index()
    {
        ...
        $this->assign(compact('onsale','forsale','warehouse','outofstock','policeforce','recycle'));
        return $this->fetch();
    }
    ...
    public function set_hot($is_hot='',$id=''){
        ...
        if($res){
            return JsonService::successful($is_hot==1 ? '置顶成功':'置顶成功');
        }else{
            return JsonService::fail($is_hot==1 ? '置顶失败':'置顶失败');
        }
    }
    ...
    public function create()
    {
        $field = [
            ...
            Form::frameImageOne('image','新品封面(750*375px)',Url::build('admin/widget.images/index',array('fodder'=>'image')))->icon('image')->width('100%')->height('500px'),
            Form::frameImages('poster_image','新品海报(750*375px)',Url::build('admin/widget.images/index',array('fodder'=>'poster_image')))->maxLength(3)->icon('images')->width('100%')->height('500px')->spin(0),
            Form::input('name','新品风格')->col(Form::col(24)),
            Form::frameImages('slider_image','新品详情(宽750px)',Url::build('admin/widget.images/index',array('fodder'=>'slider_image')))->maxLength(9)->icon('images')->width('100%')->height('500px')->spin(0),
            Form::number('browse','浏览量')->min(0)->precision(0)->col(8),
            Form::number('sort','新品排序')->col(8),
            Form::radio('is_hot','是否置顶',0)->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8),
            Form::radio('is_show','新品状态',0)->options([['label'=>'上架','value'=>1],['label'=>'下架','value'=>0]])->col(8),
        ];
    ...
    public function save(Request $request)
    {
        ...
        if(count($data['cate_id']) < 1) return Json::fail('请选择新品分类');
        $cate_id = $data['cate_id'];
        $data['mer_id'] = implode(',',$data['mer_id']);
        $data['cate_id'] = implode(',',$data['cate_id']);
        if(!$data['name']) return Json::fail('请输入新品风格');
        if(count($data['image'])<1) return Json::fail('请上传新品封面');
        if(count($data['poster_image'])<1) return Json::fail('请上传新品海报');
        if(count($data['poster_image'])>3) return Json::fail('新品海报最多3张图');
        if(count($data['slider_image'])<1) return Json::fail('请上传新品详情');
        if(count($data['slider_image'])>9) return Json::fail('新品详情最多9张图');
    ...
    public function edit($id)
    {
        ...
        $field = [
            ...
            Form::select('cate_id','新品分类',explode(',',$product->getData('cate_id')))->setOptions(function(){
                $list = CategoryModel::getTierList();
                $menus=[];
                foreach ($list as $menu){
                    $menus[] = ['value'=>$menu['id'],'label'=>$menu['html'].$menu['cate_name'],'disabled'=>$menu['pid']== 0];
            ...
            Form::frameImageOne('image','新品封面(750*375px)',Url::build('admin/widget.images/index',array('fodder'=>'image')),$product->getData('image'))->icon('image')->width('100%')->height('500px'),
            Form::frameImages('poster_image','新品海报(750*375px)',Url::build('admin/widget.images/index',array('fodder'=>'poster_image')),json_decode($product->getData('poster_image'),1) ? : [])->maxLength(3)->icon('images')->width('100%')->height('500px'),
            Form::input('name','新品风格',$product->getData('name')),
            Form::frameImages('slider_image','新品详情(宽750px)',Url::build('admin/widget.images/index',array('fodder'=>'slider_image')),json_decode($product->getData('slider_image'),1) ? : [])->maxLength(9)->icon('images')->width('100%')->height('500px'),

            Form::number('browse','浏览量',$product->getData('browse'))->min(0)->precision(0)->col(8),
            Form::number('sort','新品排序',$product->getData('sort'))->col(8),
            Form::radio('is_hot','是否置顶',$product->getData('is_hot'))->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(8),
            Form::radio('is_show','新品状态',$product->getData('is_show'))->options([['label'=>'上架','value'=>1],['label'=>'下架','value'=>0]])->col(8),
        ];
        $form = Form::make_post_form('编辑新品',$field,Url::build('update',array('id'=>$id)),2);
        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }
    ...
    public function collect($id){
        ...
        $product = ProductModel::get($id);
        if(!$product) return Json::fail('数据不存在!');
        $this->assign(NewProductRelation::getCollect($id));
        return $this->fetch();
    }
    ...
    public function like($id){
        ...
        $product = ProductModel::get($id);
        if(!$product) return Json::fail('数据不存在!');
        $this->assign(NewProductRelation::getLike($id));
        return $this->fetch();
    }
