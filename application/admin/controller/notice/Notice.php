...
class Notice extends AuthController
{
    ...
    protected $bindModel = NoticeModel::class;
    ...
    public function index($pid = 0)
    {
        ...
        $catlist = NoticeCategoryModel::where('is_del',0)->select()->toArray();
        //获取分类列表
        if($catlist){
        ...
        $this->assign('cate',NoticeCategoryModel::getTierList());
        $this->assign(NoticeModel::getAll($where));
        return $this->fetch();
    }
    ...
    public function add()
    {
        $field = [
            ...
            Form::input('directory','目录')->placeholder('多个用英文状态下的逗号隔开')->type('textarea'),
            Form::input('test','详情')->placeholder('免费部分')->type('textarea'),
    ...
    public function save(Request $request)
    {
        ...
        NoticeModel::beginTrans();
        $res1 = NoticeModel::set($data);
        if($res1)
            $res = true;
        else
            $res =false;
        NoticeModel::checkTrans($res);
        if($res)
    ...
    public function edit($id)
    {
        ...
        $field = [
            ...
            Form::input('directory','目录',$product->getData('directory'))->placeholder('多个用英文状态下的逗号隔开')->type('textarea'),
            Form::input('test','详情',$product->getData('test'))->placeholder('免费部分')->type('textarea'),
    ...
    public function edit_content($id){
        ...
        if(!$product) return Json::fail('数据不存在!');
        $this->assign([
            'content'=>NoticeModel::where('id',$id)->value('description'),
            'field'=>'description',
