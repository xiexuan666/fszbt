 ...
 */
class ConsultantCategory extends AuthController
{
    /**
     * 分类管理
     * @return mixed
    ...
    public function index(){
        ...
        $this->assign(ConsultantCategoryModel::systemPage($where));
        return $this->fetch();
    }
    ...
    public function save(Request $request){
        ...
        $new_id = $data['new_id'];
        unset($data['new_id']);
        $res = ConsultantCategoryModel::set($data);
        if(!ConsultantModel::saveBatchCid($res['id'],implode(',',$new_id))) return Json::fail('文章列表添加失败');
    ...
    public function edit($id){
        ...
        $article = ConsultantCategoryModel::get($id)->getData();
    ...
    public function update(Request $request, $id)
    {
        ...
        if($data['sort'] < 0) return Json::fail('排序不能是负数');
        $data['image'] = $data['image'][0];
        if(!ConsultantCategoryModel::get($id)) return Json::fail('编辑的记录不存在!');
        ConsultantCategoryModel::edit($data,$id);
    ...
    {
        $res = ConsultantCategoryModel::delConsultantCategory($id);
        if(!$res)
            return Json::fail(ConsultantCategoryModel::getErrorInfo('删除失败,请稍候再试!'));
        else
