...
use app\ebapi\model\supplier\Supplier as SupplierModel;
...
class Supplier extends AuthController
{
    ...
    public function admin()
    {
        ...
        $data = SupplierModel::where($where)->find();
    ...
    public function save()
    {
        ...
        if ($data['id'] && SupplierModel::be(['id' => $data['id'], 'mer_id' => $this->userInfo['uid']])) {
            $id = $data['id'];
            unset($data['id']);
            if (SupplierModel::edit($data, $id, 'id')) {
                return JsonService::successful('编辑成功!');
            } else
                return JsonService::fail('编辑失败!');
        } else {
            if ($res = SupplierModel::set($data)) {
