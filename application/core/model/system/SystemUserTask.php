...
class SystemUserTask extends ModelBasic
{
    ...
    public static function getTashList($level_id,$uid=0,$level=null,$expire=1400)
    {
       ...
       $levelData = SystemUserLevel::where('id',$level_id)->find();

       return ['reach_count'=>$reach_count,'task'=>self::tidyTask($list,$uid,$is_clear,$startTime),'levelData'=>$levelData,'userData'=>$userData];
    }
     ...
     * @热图图呢 array
     * */
    public static function tidyTask($task,$uid){
        if(!is_array($task)) return $task;
        foreach ($task as &$item){
            //如果已完成该任务进度直接为100
            $data = self::getDb('system_user_task')->where('id',$item['id'])->find();
            $count = self::getDb('user_task_finish')->where('uid',$uid)->where('task_id',$item['id'])->count();
            if($data['number'] == $count){
                $item['new_number']=$item['number'];
                $item['speed']=100;
                $item['finish']=1;
                $item['task_type_title']='完成';
            }else{
                $item['new_number'] = $count;
                $item['speed'] = bcmul(bcdiv($count,$data['number'],2),100,0);
                $item['finish'] = $count;
                $item['task_type_title']='剩余'.bcsub($data['number'],$count,0).'条';
            }
        }
        return $task;
    }
