...
class UserSign extends ModelBasic
{
    ...
    public static function sign($uid)
    {
        ...
        if(!count($sign_list)) return self::setErrorInfo('请先配置签到天数');
        $user = User::where('uid',$uid)->find();
        $sign_num = SystemConfigService::get('sx_sign_max_int');
        //检测昨天是否签到
        if(self::getYesterDayIsSign($uid)){
            $day = $sign_list[count($sign_list) -1]['day'];
            if($user->sign_num > $day) $user->sign_num=0;
        } else {
            //如果昨天没签到,回退到第一天
            $user->sign_num=0;
        }
        ...
        foreach ($sign_list as $key=>$item){
            if($user->sign_num == $item['day']){
                $sign_num = $item['sign_num'];
                self::setSignData($uid,'连续签到奖励'.$sign_num.'个',$sign_num,$user->integral);
                User::bcInc($uid,'integral',$sign_num,'uid');
            }
        }
