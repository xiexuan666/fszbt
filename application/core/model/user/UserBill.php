...
class UserBill extends ModelBasic
{
    ...
    {
        return self::where('uid',$uid)->where('category','now_money')->where('type','brokerage')->where('pm',1)->where('status',1)->whereTime('add_time', 'yesterday')->sum('number');
    }
    ...
    {
        return self::where('uid',$uid)->where('category','now_money')->where('type','brokerage')->where('pm',1)->where('status',1)->sum('number');
    }
    ...
    public static function getUserBillList($uid,$page,$limit,$type)
    {
        ...
        switch ((int)$type){
            case 0:
                $model=$model->where('type','in',['reg','charge','recharge','brokerage','pay_product','system_add','pay_product_refund','system_sub']);
                break;
            case 1:
                $model=$model->where('type','in',['reg','charge','pay_product']);
                break;
            case 2:
        ...
        foreach ($list as $item){
            ...
            $datalist=self::where('id','in',$item['ids'])->field(['FROM_UNIXTIME(add_time,"%Y-%m-%d %H:%i") as add_time','title','number','pm','link_id'])->order('add_time DESC')->select();

            foreach ($datalist as $key=>$val){
                $recharge = UserRecharge::where('id',$val['link_id'])->find();
                $datalist[$key]['send_price'] = $recharge['send_price'];
            }
            $value['list'] = $datalist;
