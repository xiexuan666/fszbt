...
use app\ebapi\model\user\UserSubion as UserSubionModel;
...
class PaymentBehavior
{
     ...
     * @return bool
     */
    public static function wechatPaySuccessSubion($orderId, $notify)
    {
        try{
            if(UserSubionModel::be(['order_id'=>$orderId,'paid'=>1,'status'=>3])) return true;
            return UserSubionModel::paySuccess($orderId);
        }catch (\Exception $e){
            return false;
        }
    }
     ...
     * @return bool
     */
    public static function wechatPaySuccessUserReg($orderId, $notify)
    {
        try{
            if(UserReg::be(['order_id'=>$orderId,'paid'=>1])) return true;
            return UserReg::regSuccess($orderId);
        }catch (\Exception $e){
            return false;
        }
    }
     ...
     * @return bool
     */
    public static function wechatPaySuccessUserCharge($orderId, $notify)
    {
        try{
            if(UserCharge::be(['order_id'=>$orderId,'paid'=>1])) return true;
            return UserCharge::paySuccess($orderId);
        }catch (\Exception $e){
            return false;
        }
    }
