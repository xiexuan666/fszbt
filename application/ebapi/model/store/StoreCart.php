...
class StoreCart extends ModelBasic
{
    ...
    public static function setCart($uid,$product_id=0,$course_id=0,$cart_num = 1,$product_attr_unique = '',$type='product',$is_new = 0,$raise_id=0,$combination_id=0,$seckill_id = 0,$bargain_id = 0)
    {
        ...
        if($seckill_id){
            ...
            if(!StoreRaise::getRaiseStock($raise_id,$cart_num))
                return self::setErrorInfo('该产品库存不足'.$cart_num);
            if(!StoreRaise::isValidRaise($raise_id))
                return self::setErrorInfo('该产品已下架或删除');

            $where = ['type'=>$type,'uid'=>$uid,'product_id'=>$product_id,'product_attr_unique'=>$product_attr_unique,'is_new'=>$is_new,'is_pay'=>0,'is_del'=>0,'raise_id'=>$raise_id];
            if($cart = self::where($where)->find()){
                $cart->cart_num = $cart_num;
                $cart->add_time = time();
                $cart->save();
                ...
                return self::set(compact('uid','product_id','cart_num','product_attr_unique','is_new','type','raise_id'));
            }
            ...
            if(!StoreCourse::isValidCourse($course_id))
                return self::setErrorInfo('该产品已下架或删除');
            if(!StoreCourseAttr::issetCourseUnique($course_id,$product_attr_unique))
                return self::setErrorInfo('请选择有效的产品属性');
            if(StoreCourse::getCourseStock($course_id,$product_attr_unique) < $cart_num)
                return self::setErrorInfo('该产品库存不足'.$cart_num);

            $where = ['type'=>$type,'uid'=>$uid,'product_id'=>$product_id,'product_attr_unique'=>$product_attr_unique,'is_new'=>$is_new,'is_pay'=>0,'is_del'=>0,'course_id'=>$course_id];
            if($cart = self::where($where)->find()){
                $cart->cart_num = $cart_num;
                $cart->add_time = time();
                $cart->save();
                ...
                return self::set(compact('uid','product_id','cart_num','product_attr_unique','is_new','type','course_id'));
            }
        }else{
    ...
    {
        return self::where('uid',$uid)->where('type','IN',$type)->where('is_pay',0)->where('is_del',0)->where('is_new',0)->count();
    }
    ...
    public static function getUserProductCartList($uid,$cartIds='',$status=0)
    {
        ...
        $raiseInfoField = 'id,image,price,postage,sales,stock,title as store_name,is_show,is_del,is_postage,cost';
        $model = new self();
        $valid = $invalid = [];
        $model = $model->where('uid',$uid)->where('type','IN','product,course')->where('is_pay',0)
            ->where('is_del',0);
        ...
        foreach ($list as $k=>$cart){
            ...
            }elseif($cart['raise_id']){
                $product = StoreRaise::field($raiseInfoField)
                    ->find($cart['raise_id'])->toArray();
            }elseif($cart['course_id']){
                $product = StoreCourse::field($productInfoField)
                    ->find($cart['course_id'])->toArray();
            }else{
            ...
            if(!$product){
                ...
                $invalid[] = $cart;
                //商品属性不对应
            }else if(!StoreCourseAttr::issetCourseUnique($cart['course_id'],$cart['product_attr_unique']) && !$cart['raise_id'] &&!$cart['combination_id'] && !$cart['seckill_id']&& !$cart['bargain_id']){
                $invalid[] = $cart;
                //正常商品
            }else if(!StoreProductAttr::issetProductUnique($cart['product_id'],$cart['product_attr_unique']) && !$cart['raise_id'] &&!$cart['combination_id'] && !$cart['seckill_id']&& !$cart['bargain_id']){
                $invalid[] = $cart;
                //正常商品
            }else{
                if($cart['course_id']){
                    if($cart['product_attr_unique']){
                        $attrInfo = StoreCourseAttr::uniqueByAttrInfo($cart['product_attr_unique']);
                        //商品没有对应的属性
                        if(!$attrInfo || !$attrInfo['stock'])
                            $invalid[] = $cart;
                        else{
                            $cart['productInfo']['attrInfo'] = $attrInfo;
                            $cart['truePrice'] = (float)StoreCourse::setLevelPrice($attrInfo['price'],$uid,true);
                            $cart['vip_truePrice'] = (float)StoreCourse::setLevelPrice($attrInfo['price'],$uid);
                        ...
                        $cart['truePrice'] = (float)StoreCourse::setLevelPrice($cart['productInfo']['price'],$uid,true);
                        $cart['vip_truePrice'] = (float)StoreCourse::setLevelPrice($cart['productInfo']['price'],$uid);
     ...
     * @throws \think\exception\DbException
     */
    public static function getUserCourseCartList($uid,$cartIds='',$status=0)
    {
        ...
        $model = new self();
        $valid = $invalid = [];
        $model = $model->where('uid',$uid)->where('type','course')->where('is_pay',0)
            ->where('is_del',0);
        if(!$status) $model->where('is_new',0);
        if($cartIds) $model->where('id','IN',$cartIds);
        $list = $model->select()->toArray();
        ...
        foreach ($list as $k=>$cart){
            ...
            if(!$product){
                ...
                $invalid[] = $cart;
                //商品属性不对应
            }else if(!StoreCourseAttr::issetCourseUnique($cart['course_id'],$cart['product_attr_unique'])){
                $invalid[] = $cart;
                //正常商品
            }else{
                if($cart['product_attr_unique']){
