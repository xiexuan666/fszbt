...
//        $totalPrice = bcadd($totalPrice,$storePostage,2);
     ...
     * @return bool|object
     */
    public static function cacheKeyCreateOrder($uid,$key,$addressId,$payType,$useIntegral = false,$couponId = 0,$mark = '',$combinationId = 0,$raiseId = 0,$courseId = 0,$pinkId = 0,$seckill_id=0,$bargain_id=0)
    {
    ...
    public static function paySuccess($orderId,$paytype='weixin',$formId = '')
    {
        ...
        foreach ($cartInfo as $k => &$cart) {
            //减库存加销量
            if ($cart['combination_id']) {
                $res2 = $res2 && StoreCombination::decCombinationStock($cart['cart_num'], $cart['combination_id']);
            } else if ($cart['course_id']) {
                $res2 = $res2 && StoreCourse::decCourseStock($cart['cart_num'], $cart['course_id']);
            } else if ($cart['raise_id']) {
                $res2 = $res2 && StoreRaise::decRaiseStock($cart['cart_num'], $cart['raise_id']);
            } else if ($cart['seckill_id']) {
                $res2 = $res2 && StoreSeckill::decSeckillStock($cart['cart_num'], $cart['seckill_id']);
            } else if ($cart['bargain_id']) {
                $res2 = $res2 && StoreBargain::decBargainStock($cart['cart_num'], $cart['bargain_id']);
            } else {
                $res2 = $res2 && StoreProduct::decProductStock($cart['cart_num'], $cart['productInfo']['id'], isset($cart['productInfo']['attrInfo']) ? $cart['productInfo']['attrInfo']['unique'] : '',$order['uid']);
            }
        }

        if($order['course_id'] && $order['raise_id'] && $order['seckill_id'] && $order['bargain_id']){
            ...
            UserBill::income('购买商品赠送积分',$order['uid'],'integral','gain',$sum_integral,$order['id'],$userInfo['integral'],'购买商品赠送'.$sum_integral.'积分');
        }
        ...
        if($order->raise_id && $res1 && !$order->refund_status) $resRaise = StoreRaisedata::createRaisedata($order);//创建众筹
