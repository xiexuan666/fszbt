...
class Supplier extends ModelBasic
{
     ...
     * @return mixed
     */
    public static function getSupplierListHot($field = 'id,title,synopsis,author,image,visit,add_time'){
        $model = new self();
     ...
     */
    public static function getSupplierLoading($field = '*',$offset = 0,$limit = 0)
    {
        $model = self::where('is_show',1)->field($field)
            ->order('sort DESC, id DESC');
        if($limit) $model->limit($offset,$limit);
        return $model->select();
    }

    public static function getSupplierList($data,$uid)
    {
        $cId = $data['cid'];
