...
class UserRecharge extends ModelBasic
{
    ...
    public static function addRecharge($uid,$price,$send_price,$member_card_id,$recharge_type = 'weixin',$paid = 0)
    {
        $order_id = self::getNewOrderId($uid);
        return self::set(compact('order_id','uid','price','send_price','member_card_id','recharge_type','paid'));
    }
    ...
    public static function rechargeSuccess($orderId)
    {
        ...
        if($order['send_price']){
            $price = bcadd($user['now_money'],bcadd($order['send_price'],$order['price'],2),2);
        } else {
            $price = bcadd($user['now_money'],$order['price'],2);
        }
        ...
        $res3 = User::edit(['now_money'=>$price],$order['uid'],'uid');
        $res = $res1 && $res2 && $res3;
        self::checkTrans($res);
        return $res;
    }
