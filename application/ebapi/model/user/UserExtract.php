...
class UserExtract extends ModelBasic
{
    ...
    protected static $extractType = ['alipay','bank','weixin','balance'];

    protected static $extractTypeMsg = ['alipay'=>'支付宝','bank'=>'银行卡','weixin'=>'微信','balance'=>'余额'];
    ...
    public static function userExtract($userInfo,$data){
        ...
        if($data['extract_type'] == 'alipay'){
            ...
            User::bcInc($userInfo['uid'],'now_money',$insertData['extract_price'],'uid');
            $mark = '提现到余额'.$insertData['balance'].'提现'.$insertData['extract_price'].'元';
            $insertData['status'] = 1;
