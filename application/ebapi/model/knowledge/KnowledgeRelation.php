...
class KnowledgeRelation extends ModelBasic
{
    use ModelTrait;
    ...
    {
        if(!$productId) return self::setErrorInfo('文章不存在!');
    ...
    public static function getUserCollectProduct($uid,$page,$limit)
    {
        $list = self::where('A.uid',$uid)
            ->field('B.id pid,B.title,B.price,B.image,B.is_show')->alias('A')
            ->where('A.type','collect')->where('A.category','product')
            ->order('A.add_time DESC')->join('__KNOWLEDGE__ B','A.product_id = B.id')
            ->page((int)$page,(int)$limit)->select()->toArray();
        foreach ($list as $k=>$product){
            if($product['pid']){
                $list[$k]['is_fail'] = $product['is_show'];
            }else{
                unset($list[$k]);
            }
        }
